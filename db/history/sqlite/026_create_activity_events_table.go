...
var m026CreateActivityEventsTable = pkgMigrate.Migration{
	...
	Up: func(db bun.IDB) error {
		...
		if _, err := db.ExecContext(context.Background(), `CREATE INDEX IF NOT EXISTS idx_activity_events_user_created ON activity_events (user_id, created_at)`); err != nil {
			return err
		}
		return nil
	},
	Down: func(db bun.IDB) error {
		if _, err := db.ExecContext(context.Background(), `DROP INDEX IF EXISTS idx_activity_events_user_created`); err != nil {
			return err
		}
		_, err := db.ExecContext(context.Background(), `DROP TABLE IF EXISTS activity_events`)
		return err
	},
}
