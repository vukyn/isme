...
// TestEmbedRoot verifies the embed FS is rooted at dist (so the server's
// html engine + asset middleware find files at the expected paths). It tolerates
// a build that ran only the .gitkeep placeholder (fresh checkout, no build-web):
// it asserts the FS opens and, when a real bundle is present, that index.html and
// the assets dir resolve.
