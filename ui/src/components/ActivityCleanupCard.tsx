...
import { LuScrollText, LuCheck, LuChevronRight, LuClock, LuCode, LuInfo, LuSave, LuSettings2 } from "react-icons/lu";
import { getActivityCleanupConfig, updateActivityCleanupConfig } from "@/apis";
...
// Default cron 0 5 * * * (05:00 daily) — avoids the rotation-cleanup job's 0 4 slot.
const DEFAULT_CRON = "0 5 * * *";
const DEFAULT_RETENTION = 90;

// Retention presets offered in the NativeSelect. Values are DAYS (activity
// history is kept far longer than rotation noise). Floor is 7 days (the backend
// rejects anything below — pruning younger than a week would risk the recent
// activity feed).
...
export const ActivityCleanupCard = () => {
	const { can } = usePermissions();
	const canUpdate = can("settings:update");

	const [loading, setLoading] = useState(true);
	const [saving, setSaving] = useState(false);
	const [saved, setSaved] = useState<SavedConfig>({ enabled: false, cron: DEFAULT_CRON, retentionDays: DEFAULT_RETENTION });
	const [lastRunAt, setLastRunAt] = useState<number | null>(null);
	const [lastPrunedCount, setLastPrunedCount] = useState<number | null>(null);
	...
	useEffect(() => {
		...
		(async () => {
			try {
				const config = await getActivityCleanupConfig();
				if (!active) return;
				setSaved({ enabled: config.enabled, cron: config.cron, retentionDays: config.retentionDays });
				setEnabled(config.enabled);
				setRetentionDays(config.retentionDays);
				setLastRunAt(config.lastRunAt);
				setLastPrunedCount(config.lastPrunedCount);
	...
	const dirty = enabled !== saved.enabled || cron !== saved.cron || retentionDays !== saved.retentionDays;
	...
	const handleSave = async () => {
		setSaving(true);
		try {
			await updateActivityCleanupConfig({ enabled, cron, retentionDays });
			setSaved({ enabled, cron, retentionDays });
	...
	// Amber chip label in the readout — retention is in DAYS, so "90d" / "365d".
	const retentionLabel = `${retentionDays}d`;
	// Relative "Last run 1d ago" label, matching the rotation card's run strip.
			...
			>
				...
				<Box lineHeight="1.3" minW="0">
					...
					<Text fontSize="12px" color="fg.muted">
						Prune activity-log history older than the retention window. Keeps the recent-activity feed lean.
					</Text>
				</Box>
				<Box
					ml="auto"
			...
			<Stack gap="22px" p="20px">
				...
				>
					<Box flex="1" minW="0" lineHeight="1.35">
						...
						<Text fontSize="12px" color="fg.muted">
							When on, old activity-log history is pruned on the schedule below.
						</Text>
					</Box>
					...
					<Switch.Root
						...
						colorPalette="purple"
					>
						<Switch.HiddenInput aria-label="Enable activity log cleanup" />
						<Switch.Control>
							<Switch.Thumb />
						</Switch.Control>
					</Switch.Root>
				...
				>
					...
					{/* Retention window — DAY-scale presets, on its own row under the Run
					...
					{/* resolved-cron + retention readout (cron chip + amber keep-Nd chip) */}
							...
							<Collapsible.Content>
								<Box px="16px" pb="16px">
									...
									<Text fontSize="12px" color="fg.muted" mt="8px">
										...
										if it no longer matches a preset. Retention is independent of the cron.
									</Text>
								</Box>
							</Collapsible.Content>
						</Box>
					...
					{lastRunLabel ? (
						<Text>
							...
							<Text as="span" color="success" fontWeight="semibold">
								pruned {(lastPrunedCount ?? 0).toLocaleString()} event{lastPrunedCount === 1 ? "" : "s"}
							</Text>
						</Text>
					) : (
				...
				>
					...
					<Text>
						...
						older than the retention window are deleted. The Welcome{" "}
						<Text as="b" color="fg.subtle" fontWeight="semibold">
							Recent activity
						</Text>{" "}
						feed reads the most recent rows and stays intact. The minimum window is 7 days.
					</Text>
				</HStack>
			</Stack>
